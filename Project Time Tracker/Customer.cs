...
namespace Project_Time_Tracker
{
    public class Customer
    {
        public static List<CustomerList> GenerateCustomerList(bool excludeHidden)
        {
            ...
            SQLiteDataReader? dr = SQLite.ExecuteReader("select CustomerID, CustomerName from Customers" + whereClause + " order by CustomerName collate nocase", SQLite.spl, "ProjectTimeTracker");
        ...
        public static void AddCustomer(string customerName)
        {
            ...
            SQLite.ExecuteNonQuery("insert into Customers (CustomerName) values (@CustomerName)", SQLite.spl, "ProjectTimeTracker");
        }
        ...
        public static int GetCustomerId(string customerName)
        {
            ...
            int customerId = 0;
            List<Tuple<string, string>> columnList = new() { Tuple.Create("CustomerName", customerName) };

            if (!SQLite.CheckValueExists("Customers", columnList, "ProjectTimeTracker"))
            {
                SQLite.spl.Add(new SQLiteParameter("@CustomerName", customerName));
                SQLite.ExecuteNonQuery("insert into Customers (CustomerName) values (@CustomerName)", SQLite.spl, "ProjectTimeTracker");
            }
